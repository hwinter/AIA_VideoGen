...
def AIA_Frame(DATABASE):
	...
	for i in range(0, len(database)):     
		...
		for fits_header_entry in entry.fits_header_entries[:100]:
			...
			if keybuff == "EXPTIME":
				exptime = '{entry.value}'.format(entry = fits_header_entry)
			if keybuff == "EXPSDEV":
				expdev = '{entry.value}'.format(entry = fits_header_entry)
		...
		if valbuff != 0:
			...
			draw.text((3468, 650),"WAVELENGTH: " + str(wavelength), font = font, fill = (b, g, r, a))
			draw.text((3468, 710),"DATE: " + str(date), font = font, fill = (b, g, r, a))
			draw.text((3468, 770),"TIME: " + str(time), font = font, fill = (b, g, r, a))
			draw.text((3468, 830),"EXPOSURE: " + str(exptime), font = font, fill = (b, g, r, a))
			draw.text((3468, 890),"EXPDEV: " + str(expdev), font = font, fill = (b, g, r, a))
			# #Turn it back in to a numpy array for OpenCV to deal with
			frameStamp = np.array(img_pil)

			print("printing frame: " + str(framenum + 1))
			cv2.imwrite(str(wavelength) + "_" + str(date) + "_" + str(time) + ".png", cv2.cvtColor(frameStamp, cv2.COLOR_RGB2BGR))
			framenum = framenum + 1
